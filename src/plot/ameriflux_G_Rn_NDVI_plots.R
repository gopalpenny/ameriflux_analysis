...
e2 <- p2 + geom_point(aes(DATE,NDVI,col=NEE))
...
for (i in 1:dim(sel.sites)[1]) {
  site <- sel.sites[i,1]
  indRnRg <- daytime$site.name == site & daytime$YEAR == 2006 & as.numeric(strftime(daytime$DATE,"%j")) %in% 190:280
...
#### FIGURE 3, Rn vs Rg
ind3ab <- daytime$site.name %in% sites.cross & strftime(daytime$DATE,"%Y") == "2006"
