...
for (site in sel.sites$sites) {
  modis.ndvi.site <- modis.ndvi[which(modis.ndvi$site.name==site),]
  ...
  names(modis.ndvi.site.daily) <- c("DATE","NDVI_interp")
...
daytime.modis.interp <- merge(daytime,modis.ndvi.daily,by=c("site.name","DATE"),all=TRUE)
daytime.modis <- merge(daytime.modis.interp,modis.ndvi,by=c("site.name","DATE"),all.x=TRUE)
daily.modis.interp <- merge(daily,modis.ndvi.daily,by=c("site.name","DATE"),all=TRUE)
daily.modis <- merge(daily.modis.interp,modis.ndvi,by=c("site.name","DATE"),all=TRUE)
